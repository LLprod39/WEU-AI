...
def task_list(request):
    base_qs = Task.objects.prefetch_related('label_relations__label', 'subtasks')
    limit = 100
...
def ai_improve_description(request, task_id):
    if request.method == 'POST':
        ...
        # Call async AI function synchronously
...
def approve_auto_execution(request, task_id):
    ...
    if request.method == 'POST':
        ...
        if success:
            ...
            # Запускаем в фоне (можно использовать celery или threading)
            import threading
