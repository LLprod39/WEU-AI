...
## «Connection stalled» и долгие шаги

Сообщение **«C: Connection stalled»** приходит от **Cursor CLI (agent)**, когда соединение с Cursor API обрывается или «зависает». Это известная проблема на стороне Cursor (форум, GitHub issues).

**Причины:** нестабильная сеть, лимиты/перегрузка Cursor API, подписка/квоты.

**Что сделано в WEU:**
- При падении шага с таким выводом в лог добавляется подсказка: проверить сеть и подписку Cursor, повторить шаг (Retry).
- Неуспешный выход агента (exit code ≠ 0) теперь корректно обрабатывается: шаг не помечается как «completed», срабатывают повторы (max_retries).
- Рекомендация: при нестабильном API увеличить `max_retries` у воркфлоу или в настройках (по умолчанию 3).

**Код выхода -1** в логах инструментов (например `pip install`, `dotnet build`) обычно значит, что команда не выполнилась в окружении (в Docker нет dotnet/Windows-инструментов, процесс убит и т.д.) — это не «Connection stalled».
