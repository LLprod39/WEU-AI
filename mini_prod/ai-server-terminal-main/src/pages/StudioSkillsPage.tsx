  ...
  return (
    <button
      ...
      className={`w-full rounded-xl border p-3 text-left transition-colors ${
        isSelected ? "border-primary bg-primary/5" : "border-border bg-card/60 hover:border-primary/40 hover:bg-muted/20"
      }`}
    >
      ...
      {skill.description && <p className="mt-2 text-[11px] text-muted-foreground">{skill.description}</p>}
      {skill.ui_hint && <p className="mt-2 text-[10px] text-muted-foreground">{skill.ui_hint}</p>}
      ...
      {skill.tags?.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {skill.tags.map((tag) => (
            <span key={tag} className="rounded bg-muted/70 px-1 py-0.5 text-[9px] text-muted-foreground">
              {tag}
            </span>
          ))}
        </div>
...
function TemplateCard({
  ...
  return (
    <button
      type="button"
      onClick={onUse}
      className="rounded-xl border border-border bg-card/60 p-3 text-left transition-colors hover:border-primary/40 hover:bg-muted/20"
    >
      <div className="flex items-center gap-2">
        <Sparkles className="h-4 w-4 text-primary" />
        <p className="text-sm font-medium text-foreground">{template.name}</p>
      </div>
      <p className="mt-2 text-[11px] text-muted-foreground">{template.description}</p>
      <p className="mt-2 text-[10px] text-muted-foreground">{template.summary}</p>
      <div className="mt-2 flex flex-wrap gap-1">
    ...
    <Card className={ok ? "border-green-500/30 bg-green-500/5" : "border-amber-500/30 bg-amber-500/5"}>
      <CardContent className="flex flex-wrap items-center justify-between gap-3 p-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-medium">{ok ? "Skill library passed validation" : "Skill library needs review"}</p>
            <p className="text-[11px] text-muted-foreground">
              {report.summary.skills} skill(s), {report.summary.errors} error(s), {report.summary.warnings} warning(s)
            </p>
          </div>
        </div>
        <Badge variant={ok ? "secondary" : "outline"} className="text-[10px]">
          {report.summary.strict ? "strict mode" : "standard mode"}
        </Badge>
      </CardContent>
    </Card>
  );
...
export default function StudioSkillsPage() {
      ...
      toast({
        description:
          response.validation.warnings.length > 0
            ? `Skill created with ${response.validation.warnings.length} warning(s)`
            : "Skill created",
      });
    },
    onError: (error: Error) => {
      ...
      toast({
        description:
          response.summary.errors > 0
            ? `Validation found ${response.summary.errors} error(s)`
            : response.summary.warnings > 0
              ? `Validation found ${response.summary.warnings} warning(s)`
              : "Skill library passed validation",
      });
    },
    onError: (error: Error) => {
      toast({ variant: "destructive", description: error.message });
  ...
  const submitWizard = () => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        description: error instanceof Error ? error.message : "Runtime policy must be valid JSON",
      });
      return;
    }
    ...
    <div className="flex h-full flex-col">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-muted-foreground">Corporate playbooks that guide MCP-backed agents</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" variant="outline" onClick={() => navigate("/studio/agents")} className="gap-1.5">
            ...
            <Card className="border-primary/20 bg-primary/5">
              <CardContent className="space-y-3 p-4">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-medium">How to assemble a corporate bot</p>
                </div>
                <div className="space-y-1 text-[11px] text-muted-foreground">
                  <p>1. MCP gives the bot hands: real tools for Keycloak, GitLab, Kubernetes, PostgreSQL, and other services.</p>
                  <p>2. Skills give the bot the company playbook: discovery, preflight, pinned context, guardrails, and reporting rules.</p>
                  <p>3. Agent Config combines model, tools, MCP servers, and selected skills into a reusable bot for admins.</p>
                </div>
              </CardContent>
            </Card>
            ...
            {templates.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <Label className="text-xs">Starter Templates</Label>
                  <Button variant="outline" size="sm" className="h-7 gap-1.5 text-[11px]" onClick={() => openCreateDialog()}>
            ...
            <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_160px]">
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder="Search skills, services, tags..."
            ...
            {!selectedSlug ? (
              <Card>
                <CardContent className="flex h-72 items-center justify-center text-center text-sm text-muted-foreground">
                  Select a skill to inspect its playbook and guardrails.
                </CardContent>
              </Card>
              ...
              <div className="space-y-4">
                <Card>
                  <CardHeader className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {selectedSkill.runtime_enforced && <Badge variant="secondary" className="text-[9px]">runtime enforced</Badge>}
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {selectedSkill.recommended_tools?.length > 0 && (
                      <div className="rounded-lg border border-border bg-muted/20 p-3">
                        <p className="text-xs font-medium">Recommended agent tools</p>
                    ...
                    {selectedSkill.runtime_enforced && (
                      <div className="rounded-lg border border-border bg-muted/20 p-3">
                        <p className="text-xs font-medium">Runtime policy</p>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base">Skill Playbook</CardTitle>
                  </CardHeader>
                  <CardContent>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-h-[90vh] max-w-4xl overflow-auto">
          <DialogHeader>
            <DialogTitle>Skill Wizard</DialogTitle>
            <DialogDescription>Create a corporate skill pack from a service template or from scratch.</DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="__none__">Blank wizard</SelectItem>
                    {templates.map((template) => (
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder="Keycloak Operations Workflow"
                  />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  placeholder="When this skill should be attached and used."
                />
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1.5">
                  ...
                  <Input value={wizard.service} onChange={(e) => setWizard((prev) => ({ ...prev, service: e.target.value }))} placeholder="keycloak" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs">Category</Label>
                  <Input value={wizard.category} onChange={(e) => setWizard((prev) => ({ ...prev, category: e.target.value }))} placeholder="Identity and Access" />
                </div>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs">UI hint</Label>
                  <Input value={wizard.ui_hint} onChange={(e) => setWizard((prev) => ({ ...prev, ui_hint: e.target.value }))} placeholder="Admin-facing instruction shown in Studio" />
                </div>
              </div>

              <div className="space-y-1.5">
                <Label className="text-xs">Tags</Label>
                <Input value={wizard.tags_text} onChange={(e) => setWizard((prev) => ({ ...prev, tags_text: e.target.value }))} placeholder="keycloak, iam, mcp, safety" />
              </div>

              <div className="space-y-1.5">
                <Label className="text-xs">Guardrail summary</Label>
                <Textarea
                  rows={2}
                  value={wizard.guardrail_summary_text}
                  onChange={(e) => setWizard((prev) => ({ ...prev, guardrail_summary_text: e.target.value }))}
                  placeholder="Requires preflight, Pins profile=test, Blocks profile switching"
                />
              </div>

              <div className="space-y-1.5">
                <Label className="text-xs">Recommended agent tools</Label>
                <Input value={wizard.recommended_tools_text} onChange={(e) => setWizard((prev) => ({ ...prev, recommended_tools_text: e.target.value }))} placeholder="report, ask_user, analyze_output" />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-border bg-muted/20 p-4">
                <div className="flex items-center gap-2">
                  <Shield className="h-4 w-4 text-primary" />
                  <p className="text-sm font-medium">Runtime policy</p>
                </div>
                <p className="mt-1 text-[11px] text-muted-foreground">
                  Keep the template default unless you know the exact original MCP tool names and pinned arguments.
                </p>
                <Textarea
                  rows={16}
              ...
              <div className="rounded-xl border border-border bg-muted/20 p-4 space-y-3">
                <p className="text-sm font-medium">Optional skill folders</p>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs font-medium">references/</p>
                    <p className="text-[10px] text-muted-foreground">Domain docs, examples, and longer procedures.</p>
                  </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground">Deterministic helpers for fragile or repetitive actions.</p>
                  </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground">Templates, brand files, and output assets.</p>
                  </div>
                  <Switch checked={wizard.with_assets} onCheckedChange={(checked) => setWizard((prev) => ({ ...prev, with_assets: Boolean(checked) }))} />
                </div>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs font-medium">Overwrite existing skill</p>
                    <p className="text-[10px] text-muted-foreground">Use only when intentionally updating the same slug.</p>
                  </div>
      ...
      <Dialog open={validateOpen} onOpenChange={setValidateOpen}>
        <DialogContent className="max-h-[85vh] max-w-3xl overflow-auto">
          <DialogHeader>
            <DialogTitle>Skill Library Validation</DialogTitle>
            <DialogDescription>Review structural and policy issues across the current Studio skill library.</DialogDescription>
          </DialogHeader>

          <div className="flex items-center justify-between gap-3 rounded-xl border border-border bg-muted/20 p-4">
            <div>
              <p className="text-sm font-medium">Validation mode</p>
              <p className="text-[11px] text-muted-foreground">Strict mode treats warnings as deployment blockers.</p>
            </div>
            <div className="flex items-center gap-2">
                ...
                <Card key={result.slug} className={result.errors.length ? "border-red-500/30" : result.warnings.length ? "border-amber-500/30" : "border-green-500/20"}>
                  <CardHeader className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {result.errors.length > 0 && <Badge variant="destructive" className="text-[10px]">{result.errors.length} errors</Badge>}
                      {result.warnings.length > 0 && <Badge variant="outline" className="text-[10px]">{result.warnings.length} warnings</Badge>}
                    </div>
                  ...
                  <CardContent className="space-y-2">
                    {result.errors.length > 0 && (
                      <div className="rounded-lg border border-red-500/30 bg-red-500/5 p-3">
                        <p className="text-xs font-medium text-red-200">Errors</p>
